...
`st_modal` — класс для создания модальных окон. Каждый экземпляр порождает дерево
...
После вызова `new st_modal(params)` в DOM появляется следующее дерево:
...
ссылку на экземпляр `st_modal` через приватный Symbol. Получить экземпляр по
элементу или CSS-селектору можно через `st_modal.find()`.
...
`this` указывает на экземпляр `st_modal`, что позволяет обращаться к его свойствам:

```js
const modal = new st_modal({
    content: `<div><p id="msg"></p><button action="close">Закрыть</button></div>`,
    before_show(data) {
        // this — экземпляр st_modal
...
Создаёт новый экземпляр `st_modal` на основе текущего.
...
### `st_modal.find(element | selector)`

Возвращает экземпляр `st_modal`, связанный с переданным DOM-элементом или CSS-селектором.

Работает для любого из четырёх элементов: `modal`, `overlay`, `container`, `content`.

```js
// Получить экземпляр по элементу <modal>
const instance = st_modal.find(document.querySelector('modal'));
instance.hide();

// Получить экземпляр по <container>
const instance2 = st_modal.find(someModal.container);

// Получить по CSS-селектору (ищет первый совпавший элемент)
const instance3 = st_modal.find('modal');
```

> **Совет:** `st_modal.find()` полезен в ситуациях, когда прямой ссылки на
...
```js
import st_modal from './st_modal.js';
...
### st_modal.find() — получение экземпляра из DOM

```js
// Где-то в коде создаётся модал
const modal = new st_modal({ content: `<div id="my-modal">...</div>` });

// В другом месте, без прямой ссылки на переменную modal:
document.addEventListener('click', e => {
    if (e.target.closest('#my-modal')) {
        const instance = st_modal.find(e.target.closest('#my-modal'));
        instance.hide();
    }
});
...
```js
const confirm_modal = new st_modal({
