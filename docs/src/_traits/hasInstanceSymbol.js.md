...
3. **Публикация API на элементе** (`expose`) — повесить методы владельца прямо на узел, чтобы с ним можно было работать, не доставая экземпляр.
...
| `expose` | `(el, api) => el` | Публикует методы владельца на самом элементе. Свойства неперечислимые (не попадают в `Object.keys` / `JSON.stringify`) и перезаписываемые. Занятое имя — нативное (`value`, `form`) или опубликованное другим владельцем — не затирается: `console.warn` и пропуск. |
