...
1. **Разрешение элементов** — превратить `Element` / CSS-селектор / HTML-строку в узел (или список узлов), с которым работает класс.
...
| `elements` | `(param) => Iterable<Element>` | Принимает `Element`, `NodeList`, `HTMLCollection`, селектор или HTML-строку. Если селектор ничего не нашёл, а строка похожа на HTML — она парсится и добавляется в `<body>`. |
